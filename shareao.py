...
class MainSHare:
    ...
    def share(self, id_post, token):
        ...
        if 'id' in response:
            print(self.format_print("Nguyễn Xuân Trường",f"{dt_now.strftime('%H:%M:%S')}: {response['id']}"))
        else:
            print(self.format_print("Nguyễn Xuân Trường",f"{dt_now.strftime('%H:%M:%S')}: SHARE POST FAILED!"))
    def run_share(self):
        while True:
            main.banner()
            try: 
